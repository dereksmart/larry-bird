...
class Larry_Bird_Comments extends WP_Comments_List_Table {
	static $instance;
	...
	function search( $results, $search_term ) {
		...
		$search_link = sprintf( ' <a href="%s" class="add-new-h2">%s</a>', $search_url, esc_html__('Search Comments', 'larry-bird') );
		$html = '<h2>' . esc_html__('Comments', 'larry-bird') . $search_link . '</h2>';
		parent::__construct();
	...
	function get_per_page( $comment_status = 'all' ) {
		...
		return apply_filters( 'larry_bird_num_results', 5 );
	}
